...
static WORD	Grp_LineBufSP_Tr[1024];
static WORD	Pal16Adr[256];			/* 16bit color パレットアドレス計算用 */
	...
	else
   {
      switch(CRTC_Regs[0x28] & 3)
      {
         case 0: /* 16 colors */
            ...
            {
               WORD temp;
               WORD *ram    = (WORD*)(&GVRAM[((adr & 0xff800) >> 1)+(adr & 0x3fe)]);
